...
unsigned int ADC_Vin,ADC_Hum,ADC_Press;
...
out_data_A=((data&0x100)>>3)+((data&0x200)>>3);
out_data_B=((data&0x80)<<3)+((data&0x70)>>4);

GPIO_SetBits(GPIOA,out_data_A);
GPIO_SetBits(GPIOB,out_data_B);

GPIO_SetBits(GPIOA,GPIO_Pin_7);       //E
delay(1);
GPIO_ResetBits(GPIOA,GPIO_Pin_7);

out_data_B=((data&0x8)<<7)+(data&0x7);
